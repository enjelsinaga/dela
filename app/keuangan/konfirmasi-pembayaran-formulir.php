...
if (isset($_GET['id_calon_siswa']) && isset($_GET['status']) && $_GET['status'] == 'lunas') {
    ...
    if ($dataCalon) {
        // Query untuk mengubah status pembayaran formulir di tabel `tabel-pembayaran-formulir`
        $queryUpdateCalon = "UPDATE `tabel-pembayaran-formulir` SET status_pembayaran_formulir = '$status_pembayaran' WHERE id_calon_siswa = '$id_calon_siswa'";

        // Eksekusi query
        if (mysqli_query($conn, $queryUpdateCalon)) {
            // Redirect ke halaman data kelola pembayaran formulir dengan pesan sukses
    ...
    // Jika parameter tidak lengkap, redirect ke halaman data kelola pembayaran formulir dengan pesan error
